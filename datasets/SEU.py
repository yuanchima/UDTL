...
def data_load(filename, fault_type):
    '''
    This function is mainly used to generate test data and training data with FFT.
    filename: Data location
    fault_type: Fault type for label lookup
    '''
    ...
    # Process middle section of the signal
    start, end = int(fl.shape[0]/2), int(fl.shape[0]/2) + signal_size
    while end <= (int(fl.shape[0]/2) + int(fl.shape[0]/3)):
        # Extract signal segment
        x = fl[start:end]
        data.append(x.reshape(-1, 1))  # Reshape to (-1, 1) for each segment
        lab.append(label)  # Use the multi-label encoding
        start += signal_size
        end += signal_size
